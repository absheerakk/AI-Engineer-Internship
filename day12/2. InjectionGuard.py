...
def is_injection_attempt(user_input: str) -> bool:
    ...
    if re.search(r"ignore\b.*?\b(instruction|previous|system|context)", cleaned):
        return True
    if re.search(r"forget\b.*?\b(context|identity|role|instruction|who you are|document assistant|assistant)", cleaned):
        return True
    if "[system]" in cleaned or "[admin]" in cleaned or "override" in cleaned:
        return True
    if "repeat your" in cleaned or "reveal your" in cleaned or "what are your instructions" in cleaned or "exact instructions" in cleaned or "translate your" in cleaned or "system prompt" in cleaned:
        return True
    if "you are now" in cleaned:
        return True
...
if submit_button:
    ...
    # Injection checker guard
    elif is_injection_attempt(question):
        st.error("Question rejected: Potential prompt injection attempt detected.")
    else:
